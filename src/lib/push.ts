...
import type { PushSubscriptionPayload } from './database.types';
...
export async function subscribeToPush(userId: string) {
  try {
    ...
    if (!subscription) {
      ...
      console.warn('Inscrição Web Push realizada com sucesso no navegador.');
    } else {
      console.warn('Navegador já inscrito no Web Push.');
    }
    ...
    const { error } = await supabase.from('push_subscriptions').upsert(
      {
        user_id: userId,
        subscription: subscription.toJSON() as unknown as PushSubscriptionPayload, // toJSON always has endpoint+keys on a valid subscription
      },
      { onConflict: 'user_id' }
    );
    ...
    if (error) {
      ...
      console.warn('Inscrição Push salva no banco de dados com sucesso.');
    }

  } catch (err: unknown) {
    if (err instanceof Error && err.name === 'AbortError') {
      console.warn('Inscrição Web Push abortada ou não permitida pelo ambiente local.');
    } else {
