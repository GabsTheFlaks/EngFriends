        ...
        if (data) {
          ...
          let formatted = data.reverse().map(msg => ({
            ...
            sender_username: Array.isArray(msg.profiles)
              ? (msg.profiles[0] as any)?.username ?? 'Desconhecido'
              : (msg.profiles as any)?.username ?? 'Desconhecido',
            sender_avatar_index: Array.isArray(msg.profiles)
              ? (msg.profiles[0] as any)?.avatar_index ?? 0
              : (msg.profiles as any)?.avatar_index ?? 0,
          }))
      ...
      if (uniqueSenderIds.length > 0) {
        await supabase.from('notifications').insert(
          uniqueSenderIds.map(recipientId => ({
            user_id: recipientId,
            title: `Nova mensagem em #${roomId}`,
            body: `${username}: ${content.slice(0, 80)}`,
            read: false,
          }))
        )
      }
