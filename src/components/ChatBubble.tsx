...
export const ChatBubble: React.FC<ChatBubbleProps> = ({ message, isMe, isDarkMode = false, onDelete, onImageClick }) => {
        ...
        >
          {message.type === 'image' ? (
            <button 
              ...
              className="block cursor-zoom-in border-none bg-transparent p-0 m-0"
            >
              <img 
                src={message.content} 
                alt="Mídia enviada" 
                className="rounded-lg max-w-full max-h-60 object-contain my-1 shadow-3xs hover:opacity-90 transition-opacity" 
              />
            </button>
          ) : (
